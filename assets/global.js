...
class QuantityInputHb extends HTMLElement {
  ...
  onInputChange(event) {
    ...
    this.querySelectorAll('button').forEach((button) =>
      button.classList.remove('quantity__selected')
    );
    this.querySelector(`.quantity__button[name='${value}']`).classList.add('quantity__selected');
  }
  ...
  onButtonClick(event) {
    ...
    if (event.target.name === 'plus') {
      ...
      this.input.value = parseInt(event.target.name)
    };
    if (previousValue !== this.input.value) this.input.dispatchEvent(this.changeEvent);
  }
