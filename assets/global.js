...
class QuantityInputHb extends HTMLElement {
  constructor() {
    super();
    this.input = this.querySelector('input');
  ...
  onButtonClick(event) {
    ...
    this.querySelector(`.quantity__option[data-quantity='${this.input.value}']`).classList.add('quantity--selected');
  }
...
class SellingPlanInputHb extends HTMLElement {
  constructor() {
    ...
    this.addEventListener('quantity-changed', (e) => {
      ...
      this.querySelectorAll(`.subscribe-price--quantity-${quantity}`).forEach((elem) => {
        elem.classList.add('active-quantity');
      })
    }) 
  }
